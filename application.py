...
class application(object):
    ...
    def get_config(self):
        ...
        try:
            ...
            if self.schema not in ("public", "ml", "schema"):
                print(
                    "Please enter a valid schema name or leave blank to backup all schemas")
                sys.exit()
        except:
            self.schema = ""
    ...
    def db_backup(self):
        ...
        self.output_filename = self.database_name + "_" + \
            self.import_string + "_" + self.date_string
        if self.schema != "":
        ...
        if self.schema == "schema":
            pg_dump('-h', 'localhost', '-U', 'postgres',
                    '-s', self.database_name, _out=self.path)
        elif self.schema != "":
            pg_dump('-h', 'localhost', '-U', 'postgres', '-F', 'c', '-Z', '9',
                    '--schema', self.schema, self.database_name, _out=self.path)
        else:
            #pg_dump('-h', 'localhost', '-U', 'postgres', '-F', 'c', '-Z', '9', self.database_name, _out=self.path)
            pg_dump('-h', 'localhost', '-U', 'postgres', '-F',
                    'c', self.database_name, _out=self.path)
        end = time.time()
